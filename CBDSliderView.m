...
- (NSString *)valueAsString {
	if (self.isInteger) {
		return [NSString stringWithFormat:@"%.f", self.slider.value];
	}
...
- (void)editValue:(id)sender {
	...
	UIAlertController *integerInputController = [UIAlertController alertControllerWithTitle:@"Edit value" message:@"" preferredStyle:UIAlertControllerStyleAlert];
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
		if ([integerInputController textFields][0].text) {
			self.slider.value = [[integerInputController textFields][0].text intValue];
		}
